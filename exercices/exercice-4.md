...
Add a domain service `AdFactory` with a method `publishAd(AdCommand adCommand, List<Ad> musicianAds, boolean isPremium)` 
that checks if a musician can post a new Ad based on their reputation and the number of existing Ads, and creates the Ad
accordingly.
